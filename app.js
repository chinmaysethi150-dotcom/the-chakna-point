...
// SMS/OTP login removed. Firebase Anonymous Auth is used so orders can still be secured by a Firebase UID.
$("continueLogin").onclick=async()=>{try{enteredPhone=$("phone").value.trim();if(!/^\+91\d{10}$/.test(enteredPhone))throw Error("Enter number like +919876543210");$("continueLogin").disabled=true;$("loginMsg").textContent="Please wait...";if(!auth.currentUser)await auth.signInAnonymously();$("loginMsg").textContent="";}catch(e){$("loginMsg").textContent=e.message||"Unable to continue.";}finally{$("continueLogin").disabled=false;}};
